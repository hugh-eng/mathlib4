...
instance [IsCancelAdd R] [IsLeftCancelMulZero R] [Mul A] [UniqueProds A] :
  ...
  mul_left_cancel_of_ne_zero {f} hf {g₁ g₂} eq := by
    ...
    have ⟨af, haf, ag, hag, uniq⟩ := UniqueProds.uniqueMul_of_nonempty hf h
...
theorem coeff_mul_add_eq_mul_of_uniqueAdd [Add A] {f g : R[A]} {a0 b0 : A}
    (h : UniqueAdd f.coeff.support g.coeff.support a0 b0) :
    (f * g).coeff (a0 + b0) = f.coeff a0 * g.coeff b0 :=
