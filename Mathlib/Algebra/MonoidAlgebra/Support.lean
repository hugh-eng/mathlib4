...
theorem support_coeff_mul_subset [DecidableEq G] (a b : MonoidAlgebra k G) :
...
lemma support_coeff_single_mul_subset [DecidableEq G] (f : MonoidAlgebra k G) (r : k) (a : G) :
    (single a r * f : MonoidAlgebra k G).coeff.support ⊆ Finset.image (a * ·) f.coeff.support := by
...
theorem support_coeff_mul_single_subset [DecidableEq G] (f : MonoidAlgebra k G) (r : k) (a : G) :
...
theorem support_coeff_single_mul_eq_image [DecidableEq G] (f : MonoidAlgebra k G) {r : k}
    (hr : ∀ y, r * y = 0 ↔ y = 0) {x : G} (lx : IsLeftRegular x) :
    (single x r * f : MonoidAlgebra k G).coeff.support = Finset.image (x * ·) f.coeff.support := by
...
theorem support_coeff_mul_single_eq_image [DecidableEq G] (f : MonoidAlgebra k G) {r : k}
...
theorem support_coeff_mul_single [IsRightCancelMul G] (f : MonoidAlgebra k G) (r : k)
    (hr : ∀ y, y * r = 0 ↔ y = 0) (x : G) :
...
theorem support_coeff_single_mul [IsLeftCancelMul G] (f : MonoidAlgebra k G) (r : k)
    (hr : ∀ y, r * y = 0 ↔ y = 0) (x : G) :
    (single x r * f : MonoidAlgebra k G).coeff.support =
...
lemma support_coeff_one_subset [One G] : (1 : MonoidAlgebra k G).coeff.support ⊆ 1 :=
  Finsupp.support_single_subset

@[to_additive (dont_translate := k) (attr := simp) support_coeff_one]
lemma support_coeff_one [One G] [NeZero (1 : k)] : (1 : MonoidAlgebra k G).coeff.support = 1 :=
...
theorem mem_span_support_coeff (f : MonoidAlgebra k G) :
...
theorem mem_span_support_coeff [AddZeroClass G] (f : k[G]) :
    f ∈ Submodule.span k (of' k G '' f.coeff.support) := by
  refine (Submodule.mem_map_equiv (e := coeffLinearEquiv _) _).1 ?_
  rw [Submodule.map_span, ← Set.image_comp]
  erw [← supported_eq_span_single, Finsupp.mem_supported]
