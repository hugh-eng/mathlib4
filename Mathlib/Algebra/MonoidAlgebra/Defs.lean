...
structure AddMonoidAlgebra (R M : Type*) [Semiring R] where
  /-- Construct an element of the additive monoid algebra `MonoidAlgebra R M`
  from its coefficients `M →₀ R`. -/
  ofCoeff ::
  /-- The coefficients `M →₀ R` of an element of the additive monoid algebra `MonoidAlgebra R M`. -/
  coeff : M →₀ R
...
structure MonoidAlgebra (R M : Type*) [Semiring R] where
  /-- Construct an element of the monoid algebra `MonoidAlgebra R M`
  from its coefficients `M →₀ R`. -/
  ofCoeff ::
  /-- The coefficients `M →₀ R` of an element of the monoid algebra `MonoidAlgebra R M`. -/
  coeff : M →₀ R
...
@[to_additive] lemma ofCoeff_coeff (x : MonoidAlgebra R M) : ofCoeff x.coeff = x := rfl
...
@[to_additive]
lemma ofCoeff_injective : (ofCoeff : (M →₀ R) → MonoidAlgebra R M).Injective :=
...
@[to_additive]
instance inhabited : Inhabited (MonoidAlgebra R M) := fast_instance% coeffEquiv.inhabited

@[to_additive]
instance nontrivial [Nontrivial R] [Nonempty M] : Nontrivial (MonoidAlgebra R M) :=
...
@[to_additive]
instance instIsCancelAdd [IsCancelAdd R] : IsCancelAdd (MonoidAlgebra R M) :=
...
instance smulZeroClass : SMulZeroClass A (MonoidAlgebra R M) :=
  fast_instance% coeffEquiv.smulZeroClass _

@[to_additive (attr := simp) (dont_translate := A) coeff_smul]
lemma coeff_smul (a : A) (x : MonoidAlgebra R M) : coeff (a • x) = a • coeff x := rfl

@[to_additive (dont_translate := A) coeff_smul_apply]
lemma coeff_smul_apply (a : A) (x : MonoidAlgebra R M) (m : M) :
    coeff (a • x) m = a • coeff x m := rfl
...
instance isScalarTower [SMulZeroClass N R] [SMulZeroClass O R] [SMul N O] [IsScalarTower N O R] :
    IsScalarTower N O (MonoidAlgebra R M) := coeffEquiv.isScalarTower ..

@[to_additive (dont_translate := N) smulCommClass]
instance smulCommClass [SMulZeroClass N R] [SMulZeroClass O R] [SMulCommClass N O R] :
    SMulCommClass N O (MonoidAlgebra R M) := coeffEquiv.smulCommClass ..
...
instance isCentralScalar [SMulZeroClass N R] [SMulZeroClass Nᵐᵒᵖ R] [IsCentralScalar N R] :
    IsCentralScalar N (MonoidAlgebra R M) := coeffEquiv.isCentralScalar _
...
@[to_additive (attr := simp)] lemma coeff_zero : coeff (0 : MonoidAlgebra R M) = 0 := rfl
@[to_additive (attr := simp)] lemma ofCoeff_zero : (ofCoeff 0 : MonoidAlgebra R M) = 0 := rfl
...
@[to_additive (attr := simp)]
lemma coeff_add (x y : MonoidAlgebra R M) : coeff (x + y) = coeff x + coeff y := rfl

@[to_additive (attr := simp)]
lemma ofCoeff_add (x y : M →₀ R) : ofCoeff (x + y) = ofCoeff x + ofCoeff y := rfl

@[to_additive (attr := simp)]
lemma coeff_sum (s : Finset ι) (f : ι → MonoidAlgebra R M) :
...
@[to_additive (attr := simp)]
lemma coeff_finsuppSum [AddCommMonoid N] (f : ι →₀ N) (g : ι → N → MonoidAlgebra R M) :
...
@[to_additive (attr := simp)]
lemma sum_coeff_single (f : MonoidAlgebra R M) : f.coeff.sum single = f := by ext; simp
...
lemma induction_linear {p : MonoidAlgebra R M → Prop} (x : MonoidAlgebra R M) (zero : p 0)
    (add : ∀ x y : MonoidAlgebra R M, p x → p y → p (x + y))
...
lemma coeff_one_one : (1 : MonoidAlgebra R M).coeff 1 = 1 := by simp [one_def]
...
lemma coeff_mul [DecidableEq M] (x y : MonoidAlgebra R M) (m : M) :
    (x * y).coeff m =
...
lemma coeff_mul_antidiag (x y : MonoidAlgebra R M) (m : M) (s : Finset (M × M))
...
lemma coeff_mul_single_of_forall_mul_ne (r : R) (x : MonoidAlgebra R M) (h : ∀ d, d * m ≠ m') :
...
lemma coeff_single_mul_of_forall_mul_ne (r : R) (x : MonoidAlgebra R M) (h : ∀ d, m * d ≠ m') :
...
@[to_additive (attr := deprecated coeff_mul_single_of_forall_mul_ne (since := "2025-08-16"))
  (dont_translate := R)]
lemma mul_single_apply_of_not_exists_mul (r : R) {g g' : M} (x : MonoidAlgebra R M)
...
@[to_additive (attr := deprecated coeff_single_mul_of_forall_mul_ne (since := "2025-08-16"))
  (dont_translate := R)]
lemma single_mul_apply_of_not_exists_mul (r : R) {g g' : M} (x : MonoidAlgebra R M)
...
lemma coeff_mul_single_one_apply (x : MonoidAlgebra R M) (r : R) (m : M) :
...
lemma coeff_single_one_mul_apply (x : MonoidAlgebra R M) (r : R) (m : M) :
    (single 1 r * x).coeff m = r * x.coeff m :=
  x.coeff_single_mul_eq_mul_coeff _ (by simp)

@[deprecated (since := "2025-08-16")] alias mul_single_one_apply := coeff_mul_single_one_apply
@[deprecated (since := "2025-08-16")] alias single_one_mul_apply := coeff_single_one_mul_apply
...
def singleOneRingHom : R →+* MonoidAlgebra R M where
  __ := singleAddHom 1
  map_one' := rfl
  map_mul' := by simp
...
lemma coeff_mul_single_mul (x : MonoidAlgebra R M) (r : R) (m m' : M) :
    (x * single m r).coeff (m' * m) = x.coeff m' * r :=
  coeff_mul_single_eq_coeff_mul _ (by simp)

@[to_additive (dont_translate := R) (attr := simp) coeff_single_mul_add]
lemma coeff_single_mul_mul (x : MonoidAlgebra R M) (r : R) (m : M) :
...
lemma coeff_mul_single_apply (x : MonoidAlgebra R G) (r : R) (g h : G) :
...
lemma coeff_single_mul_apply (x : MonoidAlgebra R G) (r : R) (g h : G) :
...
lemma coeff_mul_apply_left (x y : MonoidAlgebra R G) (g : G) :
...
lemma coeff_mul_apply_right (x y : MonoidAlgebra R G) (g : G) :
...
@[deprecated (since := "2025-08-16")] alias mul_single_apply := coeff_mul_single_apply
@[deprecated (since := "2025-08-16")] alias single_mul_apply := coeff_single_mul_apply
@[deprecated (since := "2025-08-16")] alias mul_apply_left := coeff_mul_apply_left
@[deprecated (since := "2025-08-16")] alias mul_apply_right := coeff_mul_apply_right
...
@[to_additive (dont_translate := R)]
instance addCommGroup : AddCommGroup (MonoidAlgebra R M) := fast_instance% coeffEquiv.addCommGroup

@[to_additive (attr := simp) (dont_translate := R)]
lemma coeff_neg (x : MonoidAlgebra R M) : (-x).coeff = -x.coeff := rfl
...
lemma neg_apply (m : M) (x : MonoidAlgebra R M) : (-x).coeff m = -x.coeff m := rfl
